...
client.ClientObj.prototype = {
		...
		$.ajax({
			...
			success : function(response) {
				...
				for (var i = 0; i < resultsArray.length; i++) {
					...
					$("#survey_title").text(resultsArray[i].survey_name);
					$("#survey_description").text(
							resultsArray[i].survey_description);
	...
	SendSurveyAnswers : function() {
		...
		for (var i = 0; i < questionTypeArray.length; i++) {
			...
			clientObj.GetSurveyAnswers(clientObj.getParameterByName("s"),
					GetQuestionIdArray[i], questionTypeArray[i],
					AnswerArray[i], clientObj.GetClientId());
		}
	...
	AddSurveyUsermapping : function(surveyId, userId) {
		var AnswerArray = this.GetAnswerArray();
		if (!(this.ValidateAnswers(AnswerArray))) {
			alert("Please answered all the required fields");
		} else {
			$.ajax({
				type : "POST",
