...
public class AdminController {
	...
	public Response AddNewQuestion(String DATA) {
		...
		try {
			...
			String isRequired = (json.optString("isRequired","").equals("undefined") ? "0" :  json.optString("isRequired"));
	...
	public Response GetPerSurveySummaryQuestion(String DATA) {
		...
		try {
			...
			rb = Response.ok(adminbean.GetPerSurveySummaryQuestion(questionId,surveyId,surveyAnswerType,recepientType,recepientList).toString()).build();
			if (rb.getEntity().toString().equals("[]")) {
				return Response.status(500)
						.entity("There was an error on the code kindly contact marangelo.delatorre@thomsonreuters.com")
						.build();
	...
	public void SendEmail(String DATA) {
		...
		try {
			...
			if (adminBean.SendEmailToRespondents(surveyId,message)) {
