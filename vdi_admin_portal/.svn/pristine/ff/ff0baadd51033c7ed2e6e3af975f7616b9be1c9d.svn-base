	...
	$.fn.multiselectable = function (options) {
		...
		options = $.extend({
			template: '<div class="multiselectable">' +
				...
				'<select style="height:150px" multiple="multiple" id="m-selectable"></select>' +
				'</div>' +
				'<div class="m-selectable-controls">' +
					'<button class="multis-right" value=">"></button>' +
					'<button class="multis-left" value="<"></button>' +
				'</div>' +
				'<div  class="m-selectable-to"><label for="m-selected"></label>' +
				'<select  style="height:150px" multiple="multiple" id="m-selected"></select>' +
				'</div>' +
			'</div>',
